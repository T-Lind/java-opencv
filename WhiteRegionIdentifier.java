...
public class WhiteRegionIdentifier extends OpenCvPipeline{
    /**
     ...
     */
    public WhiteRegionIdentifier(Telemetry telemetry) {
        this.telemetry = telemetry;
    }


    /**
     * Method to convert the frame to grayscale, blur it, and take the average brightness
    ...
    public Mat processFrame(Mat input){
        ...
        Imgproc.cvtColor(copyInput, copyInput, Imgproc.COLOR_BGR2GRAY);
        ...
        for(int r=frameHeight/8;r<frameHeight/2;r++){
            for(int c=frameWidth/8;c<frameWidth/5;c++){
                pixelRegionAverage += copyInput.get(r,c)[0];
            }
        }
        ...
        telemetry.addLine("Rectangular region brightness average: "+df.format(regionAverageBrightnessPercent)+"%");
        telemetry.update();
